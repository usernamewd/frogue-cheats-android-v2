...
public class Firearm extends PlayerWeapon {
    ...
    public void update(float delta) {
        // Apply rapid fire modification
        float modifiedDelta = delta;
        if (player.cheatManager != null && player.cheatManager.rapidFireEnabled) {
            modifiedDelta = delta * player.cheatManager.getRapidFireMultiplier();
        }
        ...
        if (state == State.Ready) {
            ...
            progress += modifiedDelta * reloadSpeed;
            if (progress > 1.0f) {
                progress = 0.0f;
                state = State.Ready;
                // Apply infinite ammo
                if (player.cheatManager == null || !player.cheatManager.infiniteAmmoEnabled) {
                    ammoInClip = maxAmmoInClip;
                } else {
                    // With infinite ammo, keep full clip
                    ammoInClip = maxAmmoInClip;
                    // Optionally increase total ammo pool
                    if (clips < 999) clips = 999;
                }
            }
        } else if (state == State.Firing) {
            progress += modifiedDelta * recoverySpeed;
            ...
            if (progress > 1.0f) {
                ...
                // Apply infinite ammo - don't consume ammo
                if (player.cheatManager == null || !player.cheatManager.infiniteAmmoEnabled) {
                    if (--ammoInClip <= 0) {
                        beginReload();
                        nextRoll = 0;
                    ...
                    // With infinite ammo, never run out
                    if (ammoInClip <= 0) {
                        ammoInClip = maxAmmoInClip;
                    }
                }
