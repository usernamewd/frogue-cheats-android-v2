...
//    }
     ...
     */
    private void applyCheats(float delta) {
        if (cheatManager == null) return;
        
        // Apply aim assist
        cheatManager.applyAimAssist();
        
        // Apply third person camera
        if (cheatManager.thirdPersonEnabled) {
            cheatManager.applyThirdPersonCamera();
        }
        
        // Apply bunny hop (allow rapid jumping)
        if (cheatManager.bunnyHopEnabled && hitBox.onGround && !hitBox.onObject) {
            // Allow rapid jumping by reducing cooldown
            // This affects jump timing in input handlers
        }
        
        // Apply air strafe (allow air movement)
        if (cheatManager.airStrafeEnabled && !hitBox.onGround) {
            // Modify movement to allow directional air control
            float airControlFactor = 0.5f; // 50% air control
            movement.scl(1f + airControlFactor);
        }
    }
