...
public class NPC extends GameEntity implements Pool.Poolable {
    ...
    public void render(GameWorld world) {
        // Apply wallhack: render entities even if not visible
        boolean shouldRender = isInViewDistance(world.cam, world.viewDistance);
        if (shouldRender) {
            // Check visibility unless wallhack is enabled
            if (world.player != null && world.player.cheatManager != null && world.player.cheatManager.wallhackEnabled) {
                // With wallhack, always render if in view distance (ignore frustum culling)
                shouldRender = true;
            } else {
                shouldRender = isVisible(world.cam);
            }
        }
